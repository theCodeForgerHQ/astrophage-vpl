...
#: Grid points per axis for the discrepancy sweep. Five in each of ``n_0`` and ``T_e`` gives
#: 25 L0/L1 comparisons at 65 ms per L1 Newton solve (measured) — under two seconds.
_DISCREPANCY_GRID: Final[int] = 5

#: Multiplicative span, either side of the RP-1 reference, that the discrepancy is estimated
#: over. Matches the range the sealed truth is drawn from, so the correction covers the
#: region the posterior can plausibly explore; estimating at a single point would understate
#: it everywhere else.
_DISCREPANCY_SPAN: Final[tuple[float, float]] = (0.85, 1.2)


def estimate_t2_discrepancy() -> DiscrepancyModel:
    """Estimate the structural discrepancy between the inversion's L0 and the L1 model.

    doc 05 §4 / doc 11 WBS 3.6. Sweeps a deterministic grid over the plausible ``(n_0, T_e)``
    region, pushes each point through the L0 and L1 forward chains with the same instrument,
    and takes the per-pixel RMS relative disagreement.

    The grid is deterministic rather than randomly drawn, and that is not merely tidiness.
    Sampling it on ``Stream.EXPERIMENT_DESIGN`` with the run's own seed would reproduce the
    exact factors :func:`_draw_true_theta` uses — the discrepancy would be estimated *at the
    sealed truth's own parameters*, which is precisely the accidental inverse crime doc 05 §7
    describes. A grid cannot collide with a draw.

    **A stated optimism, because otherwise it reads as more than it is.** At T2 the truth is
    generated by L1, and this estimates the discrepancy against that same L1. The correction
    is therefore matched to the actual misspecification in a way no real deployment could
    arrange: in a laboratory, reality is not L1, and an L0-vs-L1 estimate would be a *proxy*
    for L0-vs-reality rather than the thing itself.

    So a covered T2 interval demonstrates that **the mechanism works** — a coherent
    discrepancy term of the right magnitude restores calibration — and **not** that this
    project can predict its own error against nature. The honest next step is to generate the
    truth with L2 while still estimating the discrepancy from L0-vs-L1, making the estimate a
    genuine proxy; L2 exists and one truth costs a single 38-minute solve, which is
    affordable. Until that is done this is a demonstration, not a validation.
    """
    from vpl.physics.fluid.sheath import FluidSheathSolver
    ...
    def _low(theta: ControlParameters) -> FloatArray:
        params = _to_plasma_params(theta, species=species, registry=registry)
        state = inversion_solver.solve(params, grid=grid)
        return np.asarray(instrument.forward(state, window).values, dtype=np.float64)

    def _high(theta: ControlParameters) -> FloatArray:
        params = _to_plasma_params(theta, species=species, registry=registry)
        state, _ = _generate_truth(params, truth_solver=high_solver, observation_grid=grid)
        return np.asarray(instrument.forward(state, window).values, dtype=np.float64)

    factors = np.linspace(*_DISCREPANCY_SPAN, _DISCREPANCY_GRID)
    thetas = [
        reference.replace(n_0=reference.n_0 * fn, T_e=reference.T_e * ft)
        for fn in factors
        for ft in factors
    ]
    return estimate_empirical_discrepancy(low_fidelity=_low, high_fidelity=_high, thetas=thetas)
...
def _run(
    *,
    ...
    discrepancy: DiscrepancyModel | None = None,
) -> ClosedLoopReport:
    ...
    resolved_discrepancy: DiscrepancyModel = discrepancy or NoDiscrepancy()

    def _predict(u: FloatArray) -> Observable:
        theta = _theta_from_unconstrained(u, reference=reference)
        params = _to_plasma_params(theta, species=species, registry=registry)
        return inversion_instrument.forward(inversion_solver.solve(params, grid=grid), window)

    def _log_likelihood_with(u: FloatArray, coherent: FloatArray | None) -> float:
        predicted = _predict(u)
        if noise:
            return inversion_instrument.likelihood(
                measurement, predicted, coherent_discrepancy=coherent
            )
        return _t0_log_likelihood(measurement.values, predicted.values)

    # The discrepancy vector is computed ONCE, at a plug-in point, and then held fixed.
    #
    # Letting it track the trial prediction — the obvious spelling, and the one that was
    # written first — makes the likelihood's own covariance a function of theta. The
    # log-determinant then carries a term log(1 + f^2 sum(y_hat)) which varies strongly with
    # overall scale, and its curvature *adds* information about n_0. Measured on T2: the
    # posterior curvature went from 1.4e5 to 2.1e5 — the discrepancy term made the posterior
    # TIGHTER, exactly backwards. That is the standard mean-variance coupling artefact of a
    # heteroscedastic Gaussian, and the standard remedy is the plug-in estimate used here:
    # model discrepancy is a property of the model error, established a priori, not a
    # quantity to be re-derived at every trial theta.
    fixed_coherent: FloatArray | None = None
    if noise and not isinstance(resolved_discrepancy, NoDiscrepancy):
        plug_in = maximum_a_posteriori(
            log_likelihood=lambda u: _log_likelihood_with(u, None), prior=prior
        )
        basis = getattr(resolved_discrepancy, "basis", None)
        if basis is not None:
            # An empirical discrepancy already carries the observed error *directions*; its
            # covariance is absolute, so no plug-in prediction is needed and none is used.
            fixed_coherent = np.asarray(basis, dtype=np.float64)
        else:
            fixed_coherent = np.sqrt(
                np.asarray(resolved_discrepancy.variance(_predict(plug_in.unconstrained).values))
            )

    def log_likelihood(u: FloatArray) -> float:
        # sqrt of the discrepancy variance: the instrument takes a coherent standard
        # deviation, because the rank-one form is what keeps it O(n) over 1024 pixels.
        return _log_likelihood_with(u, fixed_coherent)
